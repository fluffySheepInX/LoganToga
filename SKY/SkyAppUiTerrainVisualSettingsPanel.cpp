...
namespace SkyAppSupport
{
		...
		const SkyAppPanels& panels)
	{
     ...
     const auto formatValue = [](const double value, const int32 decimals)
		...
		const auto drawValueRow = [&](const RectF& rect,
			...
			const StringView helpDescription)
			{
				...
				{
                  SimpleGUI::GetFont()(Tr(U"CommonDragToAdjust")).draw(Vec2{ rect.rightX() - 128.0, rect.y + 30.0 }, UiInternal::EditorTextOnLightSecondaryColor());
				}
			};
		const auto drawToggleRow = [&](const RectF& rect,
			...
			const StringView helpDescription)
			{
              ...
              SimpleGUI::GetFont()(enabled ? Tr(U"CommonOn") : Tr(U"CommonOff")).drawAt(stateRect.center(), ColorF{ 0.98 });
			};
		...
		if (uiEditMode)
		{
         ...
         SimpleGUI::GetFont()(Tr(U"CommonDrag")).drawAt(dragHandleRect.center(), ColorF{ 0.92, 0.96, 1.0, 0.88 });
			registerHelp(dragHandleRect,
             Tr(U"TerrainPanelMove"),
				Tr(U"TerrainPanelMoveDescription"));
		}

     if (DrawTerrainPageButton(pageButton0, Tr(U"TerrainPageSurface"), (currentPage == TerrainVisualPage::Surface)))
		{
			currentPage = TerrainVisualPage::Surface;
		}
        if (DrawTerrainPageButton(pageButton1, Tr(U"TerrainPageGrounding"), (currentPage == TerrainVisualPage::Grounding)))
		{
			currentPage = TerrainVisualPage::Grounding;
		}
      if (DrawTerrainPageButton(pageButton2, Tr(U"TerrainPageNoise"), (currentPage == TerrainVisualPage::Noise)))
		{
			currentPage = TerrainVisualPage::Noise;
		}

       registerHelp(pageButton0, Tr(U"TerrainPageSurface"), Tr(U"TerrainPageSurfaceDescription"));
		registerHelp(pageButton1, Tr(U"TerrainPageGrounding"), Tr(U"TerrainPageGroundingDescription"));
		registerHelp(pageButton2, Tr(U"TerrainPageNoise"), Tr(U"TerrainPageNoiseDescription"));
     ...
     if (DrawTextButton(resetButtonRect, Tr(U"CommonReset")))
		{
