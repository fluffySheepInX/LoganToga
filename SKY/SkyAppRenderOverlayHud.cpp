...
namespace SkyAppFlow
{
	...
	namespace OverlayDetail
	{
		...
		void DrawResourcePanelOverlay(SkyAppState& state, const SkyAppFrameState& frame)
		{
			...
			SimpleGUI::GetFont()(state.hud.uiEditMode ? U"資源 [Drag]" : (state.hud.showResourceAdjustUi ? U"資源 / 初期値" : U"資源")).draw(SkyAppUiLayout::ResourcePanelTitlePosition(resourcePanel), Palette::White);
			SimpleGUI::GetFont()(U"現在 予算 {:.0f}"_fmt(state.battle.playerResources.budget)).draw(Vec2{ static_cast<double>(resourcePanel.x + 12), static_cast<double>(resourcePanel.y + 28) }, ColorF{ 0.98, 0.90, 0.38 });
			SimpleGUI::GetFont()(U"現在 火薬 {:.0f}"_fmt(state.battle.playerResources.gunpowder)).draw(Vec2{ static_cast<double>(resourcePanel.x + 12), static_cast<double>(resourcePanel.y + 48) }, ColorF{ 0.98, 0.56, 0.42 });
			SimpleGUI::GetFont()(U"現在 魔力 {:.0f}"_fmt(state.battle.playerResources.mana)).draw(Vec2{ static_cast<double>(resourcePanel.x + 12), static_cast<double>(resourcePanel.y + 68) }, ColorF{ 0.58, 0.72, 1.0 });
