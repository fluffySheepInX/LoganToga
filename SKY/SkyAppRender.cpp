...
namespace SkyAppSupport
{
	...
	Optional<size_t> HitTestSpawnedSapper(const Array<SpawnedSapper>& spawnedSappers, const DebugCamera3D& camera)
	{
	...
	void DrawSelectedSapperRing(const DebugCamera3D& camera, const SpawnedSapper& sapper)
	{
	...
	void DrawSelectedSapperIcon(const DebugCamera3D& camera, const SpawnedSapper& sapper)
	{
	...
	void DrawSapperHealthBars(const DebugCamera3D& camera, const Array<SpawnedSapper>& spawnedSappers, const ColorF& fillColor)
	{
	...
	void DrawSpawnedSappers(const Array<SpawnedSapper>& spawnedSappers, const BirdModel& sapperModel, const ColorF& color)
	{
		for (const auto& sapper : spawnedSappers)
		{
			...
			if (sapperModel.isLoaded())
			{
				...
				const ColorF tint{ color.r * appearance, color.g * appearance, color.b * appearance, color.a };
			...
			else
			{
				...
				Sphere{ renderPosition.movedBy(0, radius, 0), radius }.draw(color.removeSRGBCurve());
			}
		}
	}
}
