...
- For LoganToga3 runtime data investigation, do not conclude missing data solely from the Visual Studio project registration file list; check the actual files under App, Git tracking status, ignore settings, and runtime resolution paths。
- In LoganToga3, mods are collections of user-defined data that include multiple story scenarios. Scenarios are story-driven skirmishes with background explanations before starting and future sound novel expressions anticipated. Users should be able to introduce mods simply by placing the mod folder alongside App/000_Warehouse/000_DefaultGame.
- For LoganToga3 mods, only one mod can be activated at a time. Inherit standard game assets explicitly in mod.toml. Mod ID and scenario ID should be case-insensitive ASCII stable IDs. Skirmishes are plain battles without stories and can coexist with story scenarios. The `--quick-battle` option allows users to skip the story and start a specific battle directly. Do not use old JSON/CSV formats。
- When considering DLL implementation for LoganToga3, provide a gateway for future planned features with unimplemented APIs, and design to log warnings for incorrect calls. DLL implementation is currently in the consideration stage。
