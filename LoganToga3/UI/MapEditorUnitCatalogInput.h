...
namespace LT3
{
    ...
    inline bool ProcessUnitCatalogEditorInput(MapEditorState& editor, UnitCatalog& catalog)
    {
        ...
        if (editor.showUnitParameterEditor && EditorUnitParameterPanelRect(editor).mouseOver())
        {
            ...
            default:
                rows = {
                    { UnitParamRowKind::Cost, U"コストを増減します" },
                    { UnitParamRowKind::Price, U"価格を増減します" },
            ...
            for (int32 rowIndex = 0; rowIndex < static_cast<int32>(rows.size()); ++rowIndex)
            {
                ...
                for (int32 buttonIndex = 0; buttonIndex < 5; ++buttonIndex)
                {
                    ...
                    case UnitParamRowKind::VisualScale:
                        ...
                        break;
                    case UnitParamRowKind::Cost:
                        if (buttonIndex == 0) adjustInt(&UnitCatalogEntry::cost, -10, 0, 99999);
                        else if (buttonIndex == 1) adjustInt(&UnitCatalogEntry::cost, 10, 0, 99999);
                        else if (buttonIndex == 2) resetInt(&UnitCatalogEntry::cost, 0);
                        break;
                    case UnitParamRowKind::Price:
                        if (buttonIndex == 0) adjustInt(&UnitCatalogEntry::price, -10, 0, 99999);
                        else if (buttonIndex == 1) adjustInt(&UnitCatalogEntry::price, 10, 0, 99999);
                        else if (buttonIndex == 2) resetInt(&UnitCatalogEntry::price, 0);
                        break;
