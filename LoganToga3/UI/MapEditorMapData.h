...
namespace LT3
{
	...
	inline void LoadMapEditorAssets(MapEditorState& editor)
	{
				...
				for (const auto tileValue : tilesValue.tableArrayView())
				{
					...
					if (decalsValue.isTableArray())
					{
						for (const auto decalValue : decalsValue.tableArrayView())
						{
							...
							asset.decalAmbientSound = decalValue[U"decal_ambient_sound"].getOr<String>(asset.decalAmbientSound);
							asset.decalAmbientVolume = Clamp(decalValue[U"decal_ambient_volume"].getOr<double>(asset.decalAmbientVolume), 0.0, 1.0);
							NormalizeDecalSettings(asset);
		...
		if (resourceToml)
		{
			TOMLTableArrayView map001Array =
				resourceToml[U"Map001"].isEmpty()
				? resourceToml[U"resources"].tableArrayView()
				: resourceToml[U"Map001"].tableArrayView();

			for (const auto resourceValue : map001Array)
			{
		...
		for (int32 y = 0; y < editor.mapHeight; ++y)
		{
			for (int32 x = 0; x < editor.mapWidth; ++x)
			{
				...
				for (const MapEditorDecalPlacement& decal : cell.decals)
				{
					...
					writer << U"decal_ambient_sound = \"" << TomlEscape(asset.decalAmbientSound) << U"\"\n";
					writer << U"decal_ambient_volume = " << Clamp(asset.decalAmbientVolume, 0.0, 1.0) << U"\n";
