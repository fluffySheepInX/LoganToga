...
void GaussianFSAddon::SetLang(const String lang)
{
	if (auto p = Addon::GetAddon<GaussianFSAddon>(U"GaussianFSAddon"))
	{
		if (p->m_lastLang != lang)
		{
			p->m_lastLang = lang;
			p->m_langChanged = true;
		}
	}
}
...
bool GaussianFSAddon::ConsumeLangChanged()
{
	if (auto p = Addon::GetAddon<GaussianFSAddon>(U"GaussianFSAddon"))
	{
		const bool changed = p->m_langChanged;
		p->m_langChanged = false;
		return changed;
	}
	return false;
}
...
void GaussianFSAddon::SetScene(const String lang)
{
	if (auto p = Addon::GetAddon<GaussianFSAddon>(U"GaussianFSAddon"))
	{
		if (p->m_lastSceneSize != lang)
		{
			p->m_lastSceneSize = lang;
			p->m_sceneChanged = true;
		}

		for (const auto& da2 : p->m_arraySceneSize)
		{
...
String GaussianFSAddon::GetScene()
{
	if (auto p = Addon::GetAddon<GaussianFSAddon>(U"GaussianFSAddon"))
	{
		return p->m_lastSceneSize;
	}
	return U"";
}

bool GaussianFSAddon::ConsumeSceneChanged()
{
	if (auto p = Addon::GetAddon<GaussianFSAddon>(U"GaussianFSAddon"))
	{
		const bool changed = p->m_sceneChanged;
		p->m_sceneChanged = false;
		return changed;
	}
	return false;
}

bool GaussianFSAddon::IsModalActive()
{
	if (auto p = Addon::GetAddon<GaussianFSAddon>(U"GaussianFSAddon"))
	{
		return (p->m_escPressed || p->m_langPressed || p->m_sceneSizePressed);
	}
	return false;
}
